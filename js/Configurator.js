...
function LoadAventador(config)
{
    ...
    // const texture1 = new THREE.TextureLoader().load( 'data/aventador/Composition-VII-1913-Kandinsky-Wassily.jpg');
...
function LoadConfigurator(mConfigJSON)
{
    ...
    '<div class="configurator-palette">',
        '<div class="options-palette">',
            '<nav class="nav-config">',
                '<ul>',
                '<li>',
                        '<a class="nav-config-item" data-id="design_colors">',
            ...
            '<div id="design_colors" class="palette-content">',  
                '<ul>',
                '</ul>',
            '</div>',
            ,
    ...
    $('.nav-config-item',config_palette).click(function()
    {
        ...
        else if(configID=='design_colors')     
        {
            var text =mConfigJSON[configID].designs[1].thumb 
            var text2 =mConfigJSON[configID].designs[0].thumb 
...
//Function to set the color of an entityt
function SetEntitytexture(targetMat ,text)
{		
	mC3DGLTF.scene.traverse(function(obj)
    {
        if(obj instanceof THREE.Mesh)
        {
            ...
            {                
                //Set the material color
                   const texture1 = new THREE.TextureLoader().load( 'data/aventador/'+text);
                 console.log(texture1);
                obj.material=new THREE.MeshStandardMaterial( { map: texture1 } );
